-- Add employee department names for MVP branch/department reporting.
-- MVP uses a simple text value instead of a department master table.

alter table public.employees
add column if not exists department_name text not null default '未設定';

create index if not exists idx_employees_company_department
on public.employees(company_id, department_name);

update public.employees e
set department_name = case e.employee_code
  when 'E-0007' then '本社営業部'
  when 'E-0008' then '本社営業部'
  when 'E-0009' then '本社営業部'
  when 'E-0010' then 'A支店'
  when 'E-0011' then 'A支店'
  when 'E-0012' then 'A支店'
  when 'E-0013' then 'B支店'
  when 'E-0014' then 'B支店'
  when 'E-0015' then 'B支店'
  else e.department_name
end
from public.companies c
where c.id = e.company_id
  and c.company_code = 'TOTAL-001'
  and e.employee_code in (
    'E-0007', 'E-0008', 'E-0009',
    'E-0010', 'E-0011', 'E-0012',
    'E-0013', 'E-0014', 'E-0015'
  );

drop function if exists public.current_employees();
...
as $$
  ...
  order by e.department_name, e.employee_code;
$$;
...
as $$
  ...
  employee_summary as (
    select
      ...
      (
        ...
        filter (where dp.id is not null and dp.status = 'valid')
      )[1] as latest_auth_method,
      coalesce(bool_or(dp.status in ('blocked_duplicate', 'voided')), false) as has_invalid_punch
    from public.employees e
    left join day_punches dp on dp.employee_id = e.id
    where e.company_id = (select id from target_company)
      and e.is_active
    group by e.id, e.employee_code, e.display_name, e.department_name
  )
  select
    es.employee_id,
    es.employee_code,
  ...
  order by es.department_name, es.employee_code;
$$;
