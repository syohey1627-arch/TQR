...
as $$
  ...
  break_pairs as (
    select
      starts.employee_id,
      starts.punched_at as break_start_at,
      (
  ...
  break_totals as (
    select
      employee_id,
      coalesce(
        floor(sum(extract(epoch from (break_end_at - break_start_at))) / 60)::integer,
        0
      ) as break_minutes
    from break_pairs
    where break_end_at is not null
